...
// 未來要實作它時需要的說明書。
//
// [決策記錄 2026-08-17 第二次] 通則寫好之後，登錄新池子的成本只剩「去書上抄兩個屬性名」。
// 這一輪一次追了三件事：登錄內力(感知+耐力)、把查克拉接上真的來源(忍者稱號)、
// 把查不到答案的屬性力查到底——**答案是「查得到，但收不了」**，記在 REJECTED_POOLS。
// 值得記下來的旁證：葵花寶典第一重寫著「若已擁有內力則使內力上限提高5點」，那個 5
// 不是這個條目自己的數字，正是本檔案 REOPEN_BONUSES 的第一項。一個獨立條目複述了通則，
// 這是「通則抄對了」目前最強的一個佐證。
//
 ...
 * 查過但收不了的記在下面的 REJECTED_POOLS，不是默默略過。
 */
export const POOL_DEFS = Object.freeze({
  ...
  內力: {
    ...
    category: "東方通用",
    recovery: "一天自行恢復完全；打坐15分鐘做一次關鍵屬性檢定，恢復等同成功數",
    sourceRef: "rules-2.35.txt 第329628行(內力.htm)：內力的關鍵屬性為感知+耐力",
  },
});

/**
 * **查過原文、但確定收不了的池子。** 這份清單跟商品包的 `examinedButRejected` 是同一個用意：
 * 一個查證過的否定結論如果不寫下來，下一個人會重新花時間查一遍，再放棄一次。
 *
 * [決策記錄 2026-08-17] 這份清單是被上一輪的錯誤逼出來的。上一輪把「查不到通用上限公式」
 * 誤判成「書上沒有」，教訓是「查不到時先去對移出清單」；這一輪查屬性力時碰到的是相反的情況——
 * **查得到，而且答案是明確的「不可能算」**。那種結論一樣要留下來，否則它跟「還沒查」
 * 在下一個人眼裡長得一模一樣。紅馬尾戰士的 pendingReason 先前寫的正是「關鍵屬性還沒查到」。
 */
export const REJECTED_POOLS = Object.freeze({
  屬性力: {
    keyAttributes: ["決心", "風度"],
    reason: "關鍵屬性兩個都不在簡化規則的六維裡(決心與風度都是被砍掉的九維之一)，算不出上限",
    sourceRef: "rules-2.35.txt 第98784行：屬性力的關鍵屬性是決心+風度",
    blocks: ["cybernetic.紅馬尾戰士.D"],
  },
  生氣: {
    keyAttributes: ["耐力", "決心"],
    reason: "關鍵屬性其中一個(決心)不在六維裡。**一個不在就夠了**——上限是兩者之和，缺一項就不是簡化，是猜",
    sourceRef: "rules-2.35.txt 第65362行：生氣的關鍵屬性為耐力+決心",
    blocks: [],
  },
});
