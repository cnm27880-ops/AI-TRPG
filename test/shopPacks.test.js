...
import { validateGood, auditGoodPricing } from "../content/shop/catalog.js";
...
import { purchase, evaluatePurchase } from "../content/shop/catalog.js";
import { weaponsFrom, checkModifiersFor, combatProfileFrom } from "../content/shop/effects.js";

const PACK_FILES = ["shop-starter-items.json", "shop-starter-feats.json", "shop-starter-bloodline.json"];
...
test("三個商品包都通過 loader 的結構驗證，type 是新登記的「商品」", () => {
  ...
  assert.deepEqual(merged.errors, [], "三個包合併不該撞名");
...
test("模板能力包：阿蘭斯買下去之後，四點屬性真的進了角色卡", () => {
  ...
  const result = purchase(c, wallet, packs[2].entries[0], { allocation: { 智力: 2, 感知: 2 } });
