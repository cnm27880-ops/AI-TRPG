...
test("XP花費公式：屬性/技能/專業/專長", () => {
  ...
  // 第3758行的公式帶有「最低為1點」的下限，第741行另有一句「技能從1級提升到2級需要1XP」，
  // 兩處一致。舊版這裡斷言 0 並在註解裡說「規則書公式字面上如此」，那是漏看下限，已於
  // 2026-08-17 更正(見 core/xp.js 的修正註解)。
  assert.equal(skillRaiseCost(1), 1); // 1->2 受下限保護，不是 (1-1)*2=0
