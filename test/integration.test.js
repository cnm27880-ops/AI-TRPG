...
test("整合：建立一個測試角色、做幾次判定、受傷、升級技能，全程狀態合理", () => {
  ...
  hero.skills["格鬥"] = 3;
  hero.skills["潛行"] = 0; // 刻意留一個0級技能，測試戰鬥/身手技能0的懲罰有沒有在整合流程裡生效
  hero.derived.hp = createHpState(20);
  hero.xp.earned = 50;

  // 1) 一次格鬥攻擊判定，DC設低一點確保容易觀察
  const attack = performCheck(hero, { attribute: "力量", skill: "格鬥", dc: 2 });
  assert.equal(attack.autoFail, false);
  assert.ok(attack.totalSuccesses >= 0);

  // 2) 一次敏捷相關的潛行判定，敏捷6應該要有1點傳奇屬性加值，且潛行0要扣1成功
  const dodge = performCheck(hero, { attribute: "敏捷", skill: "潛行", dc: 1 });
  assert.equal(dodge.flatPenaltyApplied, 1, "潛行0應該要扣1成功");
  ...
  // 4) 花費XP把格鬥從3級升到4級，確認費用公式跟角色XP池有正確互動
  const cost = skillRaiseCost(hero.skills["格鬥"]); // (3-1)*2=4
  assert.equal(cost, 4);
  assert.ok(hero.xp.earned - hero.xp.spent >= cost, "角色的XP應該要夠付這次升級");
  hero.skills["格鬥"] += 1;
  hero.xp.spent += cost;
  assert.equal(hero.skills["格鬥"], 4);
...
test("整合：心智技能為0時，performCheck直接回autoFail，不會誤觸發後續的傷害/骰子邏輯", () => {
  ...
  hero.skills["秘識"] = 0;
  const result = performCheck(hero, { attribute: "智力", skill: "秘識", dc: 1 });
