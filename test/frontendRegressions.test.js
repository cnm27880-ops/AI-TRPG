...
// ---------------------------------------------------------------------------
// [2026-09-01] 節點完成的獎勵文案必須說「獎勵點數」，不是「經驗」
//
// 伺服器在 2026-08-17 就把節點獎勵改回獎勵點數進錢包了
// （content/scenario/settlement.js 的 creditNodeReward，檔頭記著那次修正的理由），
// XP 改成只在副本通關時結算——但前端文案停在修正之前，畫面上那行字對玩家說了
// 一段時間的謊。這種漂移不會讓任何行為測試變紅：兩邊都照跑，只有文字是錯的。
// 所以把它釘成原始碼契約，跟這個檔案裡其他幾條同一種做法。
// ---------------------------------------------------------------------------
test("nodeCompleted 的通知說的是獎勵點數，不是經驗", () => {
  const notice = app.match(/if \(scenario\.nodeCompleted\)[\s\S]*?\n  \}/);
  assert.ok(notice, "找不到 nodeCompleted 的通知區塊（改過就更新這條斷言）");
  assert.match(notice[0], /點獎勵點數/);
  assert.doesNotMatch(notice[0], /點經驗/, "節點獎勵進的是錢包的 points，XP 只在通關結算時發");
});
