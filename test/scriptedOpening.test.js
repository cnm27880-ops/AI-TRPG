...
async function newSession(env, body = {}) {
  // 這組測試驗證 V2 的固定 opening contract；其後的 reference routing 由 referenceV2Smoke 覆蓋。
  const r = await readJson(await sessionPost(req(env, { draft: DRAFT, scenarioId: DEFAULT_SCENARIO_ID, ...body })));
