...
import { ripleyCooperationProfile } from "../ripleyCooperationPolicy.js";
import { parkerCooperationProfile } from "../parkerCooperationPolicy.js";
import { lambertCooperationProfile } from "../lambertCooperationPolicy.js";

export default {
  "sourceFile": "包.txt",
  "sourceTitle": "《異形：生化深淵》V2 空間資料、移動轉場與 NPC 語氣庫（P0 核心內容包）",
  "sourcePackId": "scenario.nostromo-01-v2",
  "conversionStatus": "content_only_with_approved_exploration_gap_major_variants_clue_presentations_and_luyuan_ripley_parker_lambert_npc_cooperation_pilots",
  "safetyNote": "本資料只提供玩家可見敘事、轉場旁白、已接觸 NPC 的語氣素材、依 canonical result 綁定的線索演出，以及 server-only 的陸遠／Ripley／Parker／Lambert 協作 overlay；canonical reference 與 engine effects 仍是唯一真相。重大場景、clue 與 NPC cooperation overlay 不得自行新增傷勢、物品、位置、門狀態、威脅、旗標、NPC 狀態、死亡、結局或獎勵。",
  ...
  "locations": [
    ...
    },
    {
      "id": "loc_deck_a",
      "title": "A 甲板主走廊",
      "playerVisibleDescription": "A 甲板主走廊的照明每隔幾秒暗一次。船員名錄終端停在一張沒有結尾的名單上，牆邊的動作偵測器外殼已經破損失靈。靠近通風管的牆面留著被酸液咬出的孔洞，維修通道入口的檢修梯通向更深處。",
      "atmosphere": "照明忽明忽暗，通風管深處偶爾傳來金屬摩擦聲，走廊裡瀰漫著淡淡的血腥與燒焦氣味。",
      "knownLandmarks": [
        {
          "id": "obj_crew_manifest_terminal",
          "text": "船員名錄終端，螢幕停在一張沒有結尾的失蹤名單上。"
        },
        {
          "id": "obj_motion_detector",
          "text": "牆邊的動作偵測器，外殼破損、指示燈不再閃爍。"
        },
        {
          "id": "obj_maintenance_hatch",
          "text": "維修通道入口的檢修梯，通往船體更深處。"
        }
      ],
      "playerPurpose": "確認船員下落與異形活動痕跡，決定前往科學實驗區、中央貨艙或橋樓。",
      "visibleHazardHints": [
        "牆面靠近通風管的位置有被酸液咬出的孔洞。",
        "照明間歇性失效，短暫的黑暗中無法確認走廊動靜。",
        "血痕方向不明，可能代表異形沿天花板移動。"
      ],
      "revisitVariants": [
        {
          "label": "常規回訪",
          "text": "船員名錄終端仍停在同一張名單上，動作偵測器的破損外殼沒有人修過。"
        },
        {
          "label": "高威脅／異形巡邏過後",
          "text": "通風管格柵又多了一道新的爪痕，牆面的酸液孔洞邊緣還在微微冒煙。"
        },
        {
          "label": "警報／自毀狀態",
          "text": "警報燈把整條走廊染成血紅色，照明徹底失效的間隔拉得更長，黑暗中只剩警報聲迴盪。"
        }
      ]
    },
    {
    ...
    "variants": [
      {
        ...
        "text": "陸遠的視線從手錶倒數移回你身上，槍口仍壓在安全角度。他等你說完，才用低沉短促的聲音道：「反應還行。」說完便轉身邁步，腳步刻意放輕，沒有再多說一句。",
        "npcContext": [
          "npc_luyuan"
        ],
        "allowedFacts": [
        ...
        "reviewNotes": "改綁 app_deck_luyuan_contact／自動；只補充陸遠的語氣與可觀察動作，不宣告新增裝備或關係數值。[2026-08-28修正] 原文重述了 canonical result 已經講過的「聲音與震動」「別困在沒有第二出口的房間」「指向通風管與科學區」三件事，改成只保留新反應（視線/槍口動作、簡短評語、轉身帶路），避免與 canonical result 語意重複。"
      },
      {
  ...
  "approvedNpcCooperation": {
    ...
    "npc_ripley": ripleyCooperationProfile(),
    "npc_parker": parkerCooperationProfile(),
    "npc_lambert": lambertCooperationProfile()
  }
};
