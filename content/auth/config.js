// [設計] Google 登入的設定判斷 —— 「這個部署到底有沒有把登入設好」。
//
...
/** 登入需要的三個環境變數，缺一不可。 */
const REQUIRED_VARS = ["GOOGLE_CLIENT_ID", "GOOGLE_CLIENT_SECRET", "AUTH_SESSION_SECRET"];
...
/**
 * 決定要送給 Google 的 redirect_uri。
 *
 * 這個值**必須跟 Google Cloud Console 裡登記的完全一致**，差一個斜線、
 * 差 http/https、差 www 都會被 Google 直接拒絕（而且錯誤訊息不容易看懂）。
 *
 * 預設從這次請求的網址推導，好處是 preview 部署（每次一個新網域）不用一直改設定；
 * 但 Google 的白名單不吃萬用字元，所以**正式環境請明確設定 AUTH_REDIRECT_URI**，
 * 讓它不受任何反向代理改寫 Host 標頭的影響。
 */
export function resolveRedirectUri(request, env = {}) {
  if (env.AUTH_REDIRECT_URI) return env.AUTH_REDIRECT_URI;
